...
export async function request__near_restaurants(lat, lon) {
  let response = await fetch(
    `https://tourism.opendatahub.bz.it/api/Gastronomy?pagesize=3&latitude=${lat}&longitude=${lon}&radius=300&smgactive=true`,
    {
      method: 'GET',
      headers: new Headers({
...
export async function request__near_accomodations(lat, lon) {
  let response = await fetch(
    `https://tourism.opendatahub.bz.it/api/Accommodation?pagesize=3&latitude=${lat}&longitude=${lon}&radius=300&smgactive=true`,
    {
      method: 'GET',
      headers: new Headers({
        Accept: 'application/json',
