...
export async function request__near_restaurants(lat, lon) {
  const response = await fetch(
    `${BASE_PATH}/Gastronomy?pagesize=3&latitude=${lat}&longitude=${lon}&radius=300&smgactive=true&origin=${fetch_origin}`,
    {
      method: 'GET',
      headers: new Headers({
...
export async function request__near_accomodations(lat, lon) {
  const response = await fetch(
    `${BASE_PATH}/Accommodation?pagesize=3&latitude=${lat}&longitude=${lon}&radius=300&smgactive=true&origin=${fetch_origin}`,
    {
      method: 'GET',
      headers: new Headers({
        Accept: 'application/json',
